import { LayoutDashboard,FolderOpen,BarChart2,Receipt,FileText,Truck,Users,Settings,ChevronLeft,Building2,LogOut,Menu } from 'lucide-react';

const nav = [
  { id:'dashboard', label:'Dashboard', icon:LayoutDashboard },
  { id:'proyek',    label:'Proyek',    icon:FolderOpen },
  { id:'rab',       label:'RAB & Realisasi', icon:BarChart2 },
  { id:'pengeluaran',label:'Pengeluaran',icon:Receipt },
  { id:'invoice',   label:'Invoice',   icon:FileText },
  { id:'vendor',    label:'Vendor & Supplier',icon:Truck },
  { id:'tenagakerja',label:'Tenaga Kerja',icon:Users },
];
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full z-30 flex flex-col bg-[#0F1724] text-white transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-0 lg:w-16'} overflow-hidden`}>
        ...
        <div className="flex items-center gap-3 p-4 min-h-[64px] border-b border-white/10">
          ...
          <button onClick={onToggle} className={`ml-auto p-1 hover:bg-white/10 rounded-lg transition-colors flex-shrink-0 ${isOpen ? 'flex' : 'hidden'}`}>
            <ChevronLeft size={16} />
          </button>
        </div>
        ...
        <nav className="flex-1 p-2 overflow-y-auto space-y-0.5">
          {nav.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button key={id} onClick={() => onNavigate(id)} title={!isOpen ? label : ''}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 text-left ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-white/10 hover:text-white'} ${!isOpen ? 'lg:justify-center' : ''}`}>
